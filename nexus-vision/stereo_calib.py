...
# python stereo_camera_calibration.py   --left_file left.yml \
#                                       --right_file right.yml \
#                                       --left_prefix left \
#                                       --right_prefix right \
#                                       --left_dir bothImagesFixedStereo \
#                                       --right_dir bothImagesFixedStereo \
#                                       --image_format png \
#                                       --square_size 0.025 \
#                                       --save_file stereo.yml
import numpy as np
import cv2
import glob
import argparse
...
if __name__ == '__main__':
    df_data_dir = './data/'
    df_left_file = df_data_dir + 'left.yml'
    df_right_file = df_data_dir + 'right.yml'
    df_left_prefix = 'left_cam_'
    df_right_prefix = 'right_cam_'
    df_left_dir = df_data_dir + '/calib_img/'
    df_right_dir = df_data_dir + '/calib_img/'
    df_image_format = 'jpg'
    df_width = 9
    df_height = 6
    df_square_size = 0.025
    df_save_file = df_data_dir + 'stereo.yml'

    # Check the help parameters to understand arguments
    parser = argparse.ArgumentParser(description='Camera calibration')
    parser.add_argument('--left_file', type=str, required=False, default=df_left_file, help='left matrix file')
    parser.add_argument('--right_file', type=str, required=False, default=df_right_file, help='right matrix file')
    parser.add_argument('--left_prefix', type=str, required=False, default=df_left_prefix, help='left image prefix')
    parser.add_argument('--right_prefix', type=str, required=False, default=df_right_prefix, help='right image prefix')
    parser.add_argument('--left_dir', type=str, required=False, default=df_left_dir, help='left images directory path')
    parser.add_argument('--right_dir', type=str, required=False, default=df_right_dir, help='right images directory path')
    ...
    parser.add_argument('--save_file', type=str, required=False, default=df_save_file, help='YML file to save stereo calibration matrices')
