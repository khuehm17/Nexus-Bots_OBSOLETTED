...
int Omni4WDAction::setCarSpeedMMPS(int speedMMPS)
{
	...
	if (_speedMMPS > MAXSPEED)
		_speedMMPS = MAXSPEED;
	else if (_speedMMPS < -MAXSPEED)
		_speedMMPS = -MAXSPEED;
...
{
	if (STAT_UNKNOWN <= stat && stat <= STAT_HALTED)
		_stat = stat;
	else
		_stat = STAT_UNKNOWN;
	return getStat();
...
Omni4WDAction *Omni4WDAction::add(Omni4WDAction *action)
{
	for (Omni4WDAction *act = getFirstAction(); act != NULL; act = act->getNextAction())
	{
		...
		if (act->getNextAction() == NULL)
			return act->setNextAction(action);
	}
	return NULL;
}
/*
...
unsigned char Omni4WDAction::activeAll()
{
	...
	for (Omni4WDAction *action = getFirstAction(); action != NULL; action = action->getNextAction())
	{
		if (action->getStat() == STAT_HALTED && action->active())
			++count;
	}
	return count;
}
unsigned char Omni4WDAction::countActive() const
{
	...
	for (Omni4WDAction *action = getFirstAction(); action != NULL; action = action->getNextAction())
	{
		if (action->getStat() == STAT_QUEUING || action->getStat() == STAT_ACTING)
			++count;
	}
	return count;
}
...
unsigned char Omni4WDAction::Start()
{
	...
	case Omni4WD::STAT_ADVANCE:
		getOmni4WD()->setCarAdvance(0);
		break;
	case Omni4WD::STAT_BACKOFF:
		getOmni4WD()->setCarBackoff(0);
		break;
	case Omni4WD::STAT_LEFT:
		getOmni4WD()->setCarLeft(0);
		break;
	case Omni4WD::STAT_RIGHT:
		getOmni4WD()->setCarRight(0);
		break;
	case Omni4WD::STAT_ROTATELEFT:
		getOmni4WD()->setCarRotateLeft(0);
		break;
	case Omni4WD::STAT_ROTATERIGHT:
		getOmni4WD()->setCarRotateRight(0);
		break;
	case Omni4WD::STAT_UPPERLEFT:
		getOmni4WD()->setCarUpperLeft(0);
		break;
	case Omni4WD::STAT_LOWERLEFT:
		getOmni4WD()->setCarLowerLeft(0);
		break;
	case Omni4WD::STAT_LOWERRIGHT:
		getOmni4WD()->setCarLowerRight(0);
		break;
	case Omni4WD::STAT_UPPERRIGHT:
		getOmni4WD()->setCarUpperRight(0);
		break;
	default:
		getOmni4WD()->setCarStop(0);
		break;
