...
Omni4WD::Omni4WD(MotorWheel *wheelUL, MotorWheel *wheelLL,
				 MotorWheel *wheelLR, MotorWheel *wheelUR, unsigned int wheelspan) : _wheelUL(wheelUL), _wheelLL(wheelLL),
...
{
	if (getSwitchMotorsStat() == MOTORS_BF)
		switchMotors();
	return getSwitchMotorsStat();
}
 ...
 */
// 201208
float Omni4WD::getCarSpeedRad() const
{ // Omega
...
int Omni4WD::getCarSpeedMMPS() const
{
	...
	case STAT_ROTATELEFT:
	case STAT_ROTATERIGHT:
		return abs(getCarSpeedRad() * sqrt(pow(getWheelspan() / 2, 2) * 2));
		break;
	case STAT_UNKNOWN: // Not implemented yet
		break;
	}
...
int Omni4WD::setCarSpeedMMPS(int speedMMPS, unsigned int ms)
{
	...
	case STAT_STOP:
		return currSpeed;
	case STAT_ADVANCE:
		carAction = &Omni4WD::setCarAdvance;
		break;
	case STAT_BACKOFF:
		carAction = &Omni4WD::setCarBackoff;
		break;
	case STAT_LEFT:
		carAction = &Omni4WD::setCarLeft;
		break;
	case STAT_RIGHT:
		carAction = &Omni4WD::setCarRight;
		break;
	case STAT_ROTATELEFT:
		carAction = &Omni4WD::setCarRotateLeft;
		break;
	case STAT_ROTATERIGHT:
		carAction = &Omni4WD::setCarRotateRight;
		break;
	case STAT_UPPERLEFT:
		carAction = &Omni4WD::setCarUpperLeft;
		break;
	case STAT_LOWERLEFT:
		carAction = &Omni4WD::setCarLowerLeft;
		break;
	case STAT_LOWERRIGHT:
		carAction = &Omni4WD::setCarLowerRight;
		break;
	case STAT_UPPERRIGHT:
		carAction = &Omni4WD::setCarUpperRight;
		break;
	}
	...
	for (int time = 20, speed = currSpeed; (unsigned int)time <= ms; time += 20)
	{
...
int Omni4WD::wheelULSetSpeedMMPS(int speedMMPS)
...
int Omni4WD::wheelLLSetSpeedMMPS(int speedMMPS)
{ // direction sensitive, 201208
...
int Omni4WD::wheelLRSetSpeedMMPS(int speedMMPS)
{ // direction sensitive, 201208
...
int Omni4WD::wheelURSetSpeedMMPS(int speedMMPS)
{ // direction sensitive, 201208
...
bool Omni4WD::PIDDisable()
{
	...
	_wheelUL->PIDDisable();
	_wheelUL->runPWM(0, DIR_ADVANCE);
	_wheelLL->PIDDisable();
	_wheelLL->runPWM(0, DIR_ADVANCE);
	_wheelLR->PIDDisable();
	_wheelLR->runPWM(0, DIR_ADVANCE);
	_wheelUR->PIDDisable();
	_wheelUR->runPWM(0, DIR_ADVANCE);
	return false;
}
 ...
 */
void Omni4WD::delayMS(unsigned int ms, bool debug, unsigned char *actBreak)
{ // 201209
